...
ifstream media_ps_file;
string media_ps_str = "";

media_ps_file.open("media_aritmetica_ps.txt");

	if (media_ps_file.fail()) {
		cout << "errore nell'apertura del file" << endl;	
	}

	getline (media_ps_file, media_ps_str);
	
	try {
	media_ps = stod(media_ps_str);
	}
	catch (invalid_argument){
	cout << "errore in stod" << endl;
	}
	
	media_ps_file.close();
	
double scarto_if_ps = 0;
...
//confronto misure con tre sigma
	...
	//ciclo di lettura delle misure 
		...
		while (!file_in_misure_ps.eof()) {
			...
			scarto_if_ps = abs(media_ps - valore_in_lettura);
			
			  
			if (scarto_if_ps > sigma_3){
...
ifstream media_pt_file;
string media_pt_str = "";

media_pt_file.open("media_aritmetica_pt.txt");

	if (media_pt_file.fail()) {
		cout << "errore nell'apertura del file" << endl;	
	}

	getline (media_pt_file, media_pt_str);
	
	try {
	media_pt = stod(media_pt_str);
	}
	catch (invalid_argument){
	cout << "errore in stod" << endl;
	}
	...
	//ciclo di lettura delle misure 
		...
		while (!file_in_misure_pt.eof()) {
			...
			ciclo_while_in_lettura = ciclo_while_in_lettura + 1;  
			  ...
			  scarto_if_pt = abs(media_pt - valore_in_lettura);
			  
			if (scarto_if_pt > sigma_3){
...
ifstream media_pl2_file;
string media_pl2_str = "";

media_pl2_file.open("media_aritmetica_pt.txt");

	if (media_pl2_file.fail()) {
		cout << "errore nell'apertura del file" << endl;	
	}

	getline (media_pl2_file, media_pl2_str);
	
	try {
	media_pl2 = stod(media_pl2_str);
	}
	catch (invalid_argument){
	cout << "errore in stod" << endl;
	}
	
	media_pl2_file.close();
	
double scarto_if_pl2 = 0;
...
//confronto misure con tre sigma
	...
	//ciclo di lettura delle misure 
		...
		while (!file_in_misure_pl2.eof()) {
			...
			scarto_if_pl2 = abs(media_pl2 - valore_in_lettura);
			  
			if (scarto_if_pl2 > sigma_3){
...
//apertura file in lettura	
	...
	getline (file_in_g_ps, periodo_ps_str);
	try {
	periodo_ps = stod(periodo_ps_str);
	}
	catch (invalid_argument){
...
//esecuzione operazioni
	...
	periodo_ps_2 = pow(periodo_ps,2);
	parametro_pi = 2 * pi_greco;
	parametro_pi_2 = pow(parametro_pi,2);
	parametro_lt = lunghezza / periodo_ps_2;
...
//----------------------------------------------------------------------------------
	...
	file_out_parametri << periodo_ps_2 << endl << endl;
...
//--incertezza periodo
    double incertezza_periodo_ps = 0;
    // la variabilr periodo è stata definita precedentemente nel codice come     periodo_ps per il periodo singolo
	
    //input dell'incertezza del periodio
    
    ifstream file_in_incertezza_periodo;
    string valore_incertezza_periodo_ps_str = "";
	...
	getline (file_in_incertezza_periodo, valore_incertezza_periodo_ps_str);
	
	try {
	incertezza_periodo_ps = stod(valore_incertezza_periodo_ps_str);
	}
	catch (invalid_argument){
	cout << "Errore lettura valore" << endl;
	}
...
//--definizione delle variabili
	...
	periodo_ps_quadro = pow(periodo_ps,2);
	sotto_parametro_t = incertezza_periodo_ps / periodo_ps_quadro;
...
//------------------------------------------------------------------------------------------------------------
	file_out_incertezza_g_parametri.open("g_incertezza_ps_parametri.txt");
		...
		file_out_incertezza_g_parametri << incertezza_periodo_ps << endl << endl;
...
//variabili numeriche
     ...
     lunghezza = 0;
     double periodo_pt = 0;
     double periodo_pt_2 = 0;
     g_stima = 0;
     parametro_pi = 0;
     parametro_pi_2 = 0;
     parametro_lt = 0;
    
     double periodo_pt_3 = 0;

//variabili tipo stringa
     string periodo_pt_str = "";
...
//apertura file in lettura	
	...
	getline (file_in_g_pt, periodo_pt_str);
	try {
	periodo_pt_3 = stod(periodo_pt_str);
	}
	catch (invalid_argument){
	cout << "Errore lettura valore" << endl;
	}
...
//esecuzione operazioni
	...
	periodo_pt = periodo_pt_3 / 3;

	periodo_pt_2 = pow(periodo_pt,2);
	parametro_pi = 2 * pi_greco;
	parametro_pi_2 = pow(parametro_pi,2);
	parametro_lt = lunghezza / periodo_pt_2;
	g_stima = parametro_pi_2 * parametro_lt;
...
//----------------------------------------------------------------------------------
	...
	file_out_parametri << "Quadrato del periodo" << endl;
	file_out_parametri << periodo_pt_2 << endl << endl;
	file_out_parametri << "Parametro 2pi" << endl;
	file_out_parametri << parametro_pi << endl << endl;
	file_out_parametri << "Parametro 2pi quadrato" << endl;
	file_out_parametri << parametro_pi_2 << endl << endl;
...
//--incertezza periodo
    double incertezza_periodo_pt = 0;
    // la variabilr periodo è stata definita precedentemente nel codice come     periodo_pt
	
    //input dell'incertezza del periodio
    
    ifstream file_in_incertezza_periodo_pt;
    string valore_incertezza_periodo_pt_str_yyy = "";
	
    	file_in_incertezza_periodo_pt.open ("sem_pt.txt");
    	
    		//controllo apertura file
		if (file_in_incertezza_periodo_pt.fail()){
		cout << "Errore nell'apertura del file di output sem pt" << endl; 
		}

	getline (file_in_incertezza_periodo_pt, valore_incertezza_periodo_pt_str_yyy);
	
	try {
	incertezza_periodo_pt = stod(valore_incertezza_periodo_pt_str_yyy);
	}
	catch (invalid_argument){
	cout << "Errore lettura valore" << endl;
	}

file_in_incertezza_periodo_pt.close();

//cout << incertezza_periodo_pt << endl;
	
//calcolo dell'incertezza di g

	double incertezza_periodo_pt_diviso_3 = 0;
	
	incertezza_periodo_pt_diviso_3 = incertezza_periodo_pt / 3;
...
//--definizione delle variabili
	...
	double periodo_quadro_pt = 0;
	
	parametro_l = incertezza_l / lunghezza;
	parametro_l_quadro = pow(parametro_l,2);
	
	//devo mettere il periodo singolo
	
	
	
	periodo_quadro_pt = pow(periodo_pt,2);
	sotto_parametro_t = incertezza_periodo_pt_diviso_3 / periodo_quadro_pt;
	parametro_t = 2 * sotto_parametro_t;
	parametro_t_quadro = pow(parametro_t,2);
...
//-----------------------------------------------------------------------------------------------------------	
	file_out_incertezza_l.open("incertezza_lunghezza.txt");
		...
		//controllo paertura file
		if (file_out_incertezza_l.fail()){
		cout << "Errore nell'apertura del file di output inc lungh" << endl; 
		}
...
//-----------------------------------------------------------------------------------------------------------	
	file_out_incertezza_g.open("incertezza_g_pt.txt");
		...
		//controllo paertura file
		if (file_out_incertezza_g.fail()){
		cout << "Errore nell'apertura del file di output inc g " << endl; 
		}
...
//------------------------------------------------------------------------------------------------------------
	file_out_incertezza_g_parametri.open("g_incertezza_pt_parametri.txt");
		...
		//controllo paertura file
		if (file_out_incertezza_g_parametri.fail()){
		cout << "Errore nell'apertura del file di output param" << endl; 
		}
		...
		file_out_incertezza_g_parametri << radice_quadrata_di6 << endl << endl;
		file_out_incertezza_g_parametri << "VERIFICA CORRETTA ACCUISIZIONE DELL'INCERTEZZA SUL PERIODO" << endl;
		file_out_incertezza_g_parametri << incertezza_periodo_pt << endl << endl;
		file_out_incertezza_g_parametri << "PARAMETRI NEL CALCOLO DELL'INGERTEZZA DI g" << endl;
		file_out_incertezza_g_parametri << "parametro incertezza di l diviso l" << endl;
		file_out_incertezza_g_parametri << parametro_l << endl;
		file_out_incertezza_g_parametri << "quadrato del numero sopra" << endl;
...
//variabili numeriche
     ...
     pi_greco = M_PI;
     lunghezza = 0;
     double periodo_pl2 = 0;
     double periodo_pl2_2 = 0;
     g_stima = 0;
     parametro_pi = 0;
     parametro_pi_2 = 0;
     parametro_lt = 0;
...
//apertura file in lettura	
	...
	getline (file_in_g_pl2, periodo_pl2_str);
	try {
	periodo_pl2 = stod(periodo_pl2_str);
	}
	catch (invalid_argument){
	cout << "Errore lettura valore" << endl;
	}
...
//esecuzione operazioni
	...
	periodo_pl2_2 = pow(periodo_pl2,2);
	parametro_pi = 2 * pi_greco;
	parametro_pi_2 = pow(parametro_pi,2);
	parametro_lt = lunghezza / periodo_pl2_2;
	g_stima = parametro_pi_2 * parametro_lt;
...
//----------------------------------------------------------------------------------
	...
	file_out_parametri << "Quadrato del periodo" << endl;
	file_out_parametri << periodo_pl2_2 << endl << endl;
	file_out_parametri << "Parametro 2pi" << endl;
	file_out_parametri << parametro_pi << endl << endl;
	file_out_parametri << "Parametro 2pi quadrato" << endl;
	file_out_parametri << parametro_pi_2 << endl << endl;
...
//--incertezza periodo
    double incertezza_periodo_pl2 = 0;
    // la variabilr periodo è stata definita precedentemente nel codice come     periodo
	
    //input dell'incertezza del periodio
    
    ifstream file_in_incertezza_periodo_pl2;
    string valore_incertezza_periodo_pl2_str = "";
	
    	file_in_incertezza_periodo_pl2.open ("sem_pl2.txt");
    	
    		//controllo paertura file
		if (file_in_incertezza_periodo_pl2.fail()){
		cout << "Errore nell'apertura del file di output" << endl; 
		}

	getline (file_in_incertezza_periodo_pl2, valore_incertezza_periodo_pl2_str);
	
	try {
	incertezza_periodo_pl2 = stod(valore_incertezza_periodo_pl2_str);
	}
	catch (invalid_argument){
	cout << "Errore lettura valore" << endl;
	}
	
	file_in_incertezza_periodo_pl2.close();
	
//cout << incertezza_periodo_pl2 << endl;
...
//--definizione delle variabili
	...
	periodo_pl2_quadro = pow(periodo_pl2,2);
	sotto_parametro_t = incertezza_periodo_pl2 / periodo_pl2_quadro;
	parametro_t = 2 * sotto_parametro_t;
	parametro_t_quadro = pow(parametro_t,2);
...
//------------------------------------------------------------------------------------------------------------
	file_out_incertezza_g_parametri.open("g_incertezza_pl2_parametri.txt");
		...
		file_out_incertezza_g_parametri << radice_quadrata_di6 << endl << endl;
		file_out_incertezza_g_parametri << "VERIFICA CORRETTA ACCUISIZIONE DELL'INCERTEZZA SUL PERIODO" << endl;
		file_out_incertezza_g_parametri << incertezza_periodo_pl2 << endl << endl;
		file_out_incertezza_g_parametri << "PARAMETRI NEL CALCOLO DELL'INGERTEZZA DI g" << endl;
		file_out_incertezza_g_parametri << "parametro incertezza di l diviso l" << endl;
		file_out_incertezza_g_parametri << parametro_l << endl;
		file_out_incertezza_g_parametri << "quadrato del numero sopra" << endl;
...
cout << "cavendish ps = " << costante_cavendish_ps << endl;

//INCERTEZZA SU CAVENDISH

double incertezza_cavendish_ps = 0;
double incertezza_g_xcav_ps = 0;

ifstream file_in_inc_g_xcav_ps;
ofstream file_out_inc_cav_ps;

string incertezza_g_xcav_ps_str = "";

file_in_inc_g_xcav_ps.open("incertezza_g_ps.txt");

	if (file_in_inc_g_xcav_ps.fail()){
	cout << "Errore nell'apertura del file di input" << endl;
	}

	getline (file_in_inc_g_xcav_ps, incertezza_g_xcav_ps_str);
	try {
	incertezza_g_xcav_ps = stod(incertezza_g_xcav_ps_str);
	//cout << "controllo  " << incertezza_g_xcav_ps << endl;	
	}
	catch (invalid_argument){ 
	
	}

file_in_inc_g_xcav_ps.close();

//calcoli
incertezza_cavendish_ps = parametro * incertezza_g_xcav_ps;

//stampa su file

file_out_inc_cav_ps.open("incertezza_cavendish_ps.txt");

file_out_inc_cav_ps << incertezza_cavendish_ps;

file_out_inc_cav_ps.close();
...
cout << "cavendish pt = " << costante_cavendish_pt << endl;

//INCERTEZZA SU CAVENDISH

double incertezza_cavendish_pt = 0;
double incertezza_g_xcav_pt = 0;

ifstream file_in_inc_g_xcav_pt;
ofstream file_out_inc_cav_pt;

string incertezza_g_xcav_pt_str = "";

file_in_inc_g_xcav_pt.open("incertezza_g_pt.txt");

	if (file_in_inc_g_xcav_pt.fail()){
	cout << "Errore nell'apertura del file di input" << endl;
	}

	getline (file_in_inc_g_xcav_pt, incertezza_g_xcav_pt_str);
	try {
	incertezza_g_xcav_pt = stod(incertezza_g_xcav_pt_str);
	//cout << "controllo  " << incertezza_g_xcav_pt << endl;	
	}
	catch (invalid_argument){ 
	
	}

file_in_inc_g_xcav_pt.close();

//calcoli
incertezza_cavendish_pt = parametro * incertezza_g_xcav_pt;

//stampa su file

file_out_inc_cav_pt.open("incertezza_cavendish_pt.txt");

file_out_inc_cav_pt << incertezza_cavendish_pt;

file_out_inc_cav_pt.close();
...
cout << "cavendish pl2 = " << costante_cavendish_pl2 << endl << endl;


//INCERTEZZA SU CAVENDISH

double incertezza_cavendish_pl2 = 0;
double incertezza_g_xcav_pl2 = 0;

ifstream file_in_inc_g_xcav_pl2;
ofstream file_out_inc_cav_pl2;

string incertezza_g_xcav_pl2_str = "";

file_in_inc_g_xcav_pl2.open("incertezza_g_pl2.txt");

	if (file_in_inc_g_xcav_pl2.fail()){
	cout << "Errore nell'apertura del file di input" << endl;
	}

	getline (file_in_inc_g_xcav_pl2, incertezza_g_xcav_pl2_str);
	try {
	incertezza_g_xcav_pl2 = stod(incertezza_g_xcav_pl2_str);
	//cout << "controllo  " << incertezza_g_xcav_pl2 << endl;	
	}
	catch (invalid_argument){ 
	
	}

file_in_inc_g_xcav_pl2.close();

//calcoli
incertezza_cavendish_pl2 = parametro * incertezza_g_xcav_pl2;

//stampa su file

file_out_inc_cav_pl2.open("incertezza_cavendish_pl2.txt");

file_out_inc_cav_pl2 << incertezza_cavendish_pl2;

file_out_inc_cav_pl2.close();
...
//import dati
	...
	cout << "Errore nell'apertura del file CAV PS" << endl;
	}
	...
	cout << "Errore nell'apertura del file INC CAV PS" << endl;
	}
...
//import dati
	...
	//controllo apertura file
	if (file_in_cavendish.fail()){
	cout << "Errore nell'apertura del file CAV PL2" << endl;
	}
	...
	//controllo apertura file
	if (file_in_incertezza_cavendish.fail()){
	cout << "Errore nell'apertura del file INC CAV PL2" << endl;
	}
