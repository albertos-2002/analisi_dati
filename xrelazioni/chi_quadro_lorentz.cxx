...
//parametri di fit della curva di lorentz
long double parametro_numl = 0.00120368/0.191639;
long double parametro_wo = pow(0.961265,2);
long double parametro_gamma = pow(0.00772994,2);
...
for (int i = 0; i<numerosita_campione; ++i) {
	...
	keep1 = parametro_wo - pow(misure_x.at(i),2);
	keep2 = pow(keep1,2);
	keep3 = 4*parametro_gamma*pow(misure_x.at(i),2);
	keep4 = keep3+keep2;
	keep5 = sqrt(keep4);

	keep6 = parametro_numl / keep5;

	valori_riferimento.push_back(keep6);
}
